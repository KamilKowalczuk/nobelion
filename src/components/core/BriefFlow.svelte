<script lang="ts">
  ...
  const annualCost = $derived(
    formData.hoursWeek > 0 && formData.peopleInvolved
      ? Math.round(
          formData.hoursWeek *
            peopleMultiplier(formData.peopleInvolved) *
            100 *
            52,
        )
      : 0,
  ...
  function canProceed(step: number): boolean {
    switch (step) {
      case 1:
        return formData.diagnosis !== null;
      case 2:
        return formData.industry !== "" && formData.size !== "";
      case 3:
        return formData.problem.trim().length >= 30;
      case 4:
        return formData.peopleInvolved !== "" && formData.growsWithScale !== "";
      case 5:
        return true;
      case 6:
        return formData.urgency !== "" && formData.scope !== "";
      case 7:
        return (
  ...
  const diagnosisOptions = [
    {
      id: "biuro",
      title: "POWTARZALNA PRACA BIUROWA",
      body: "kopiowanie danych, ręczne maile, raporty",
    },
    {
      id: "strona",
      title: "STRONA NIE SPRZEDAJE LUB JEJ NIE MAM",
      body: "potrzebuję czegoś co konwertuje",
    },
    {
      id: "sprzedaz",
      title: "SPRZEDAŻ POTRZEBUJE BOOSTU",
      body: "lead generation, outreach, follow-up",
    },
    {
      id: "wizja",
      title: "MAM WIZJĘ, POTRZEBUJĘ EXEKUCJI",
      body: "wiem czego chcę, szukam wykonawcy",
    },
  ];
  ...
  const scopeOptions = [
    {
      value: "mvp",
      label: "TYLKO MVP",
      desc: "najszybciej, działa, koszt minimalny",
    },
    { value: "pelny", label: "PEŁNY SYSTEM", desc: "solidnie, długoterminowo" },
    {
      value: "doradzcie",
      label: "NIE WIEM, DORADZCIE",
      desc: "pomóżcie wybrać",
    },
  ];
  const budgetOptions = [
    "< 5 000 zł",
  ...
  > = {
    1: {
      label: "01 / 07 — DIAGNOZA",
      heading: "ZACZNIJMY OD BÓLU.",
      body: "Wybierz najbliższe Twojej sytuacji. Możesz później doprecyzować.",
    },
    2: {
      label: "02 / 07 — KONTEKST",
      heading: "POZNAJEMY TWÓJ ŚWIAT.",
      body: "Dzięki temu nie pytamy o oczywiste rzeczy później.",
    },
    3: {
      label: "03 / 07 — PROBLEM",
      heading: "WŁASNYM JĘZYKIEM. BEZ ŻARGONU.",
      body: "Im konkretniej, tym lepsza wycena. Pisz jak do kolegi.",
    },
    4: {
      label: "04 / 07 — SKALA",
      heading: "POMOŻEMY ZMIERZYĆ.",
      body: "Kwantyfikujemy problem, żebyśmy wiedzieli, jaki ROI ma sens.",
    },
    5: {
      label: "05 / 07 — KONTEKST 2",
      heading: "NIC NIE OCENIAMY.",
      body: "Wiedza o tym, co nie zadziałało, oszczędza nam (i Tobie) tygodni pracy.",
    },
    6: {
      label: "06 / 07 — RAMY",
      heading: "USTALMY OCZEKIWANIA.",
      body: "Wycena zależy od zakresu i pilności. Powiedz szczerze, dopasujemy się.",
    },
    7: {
      label: "07 / 07 — FINISZ",
      heading: "OSTATNI KROK.",
      body: "Po wysłaniu odzywamy się w ciągu 24h roboczych. Nie sprzedajemy. Po prostu rozmawiamy o tym, czy mamy sens.",
    },
  };
</script>

<div
  class="briefflow w-full bg-surface/40 border border-white/10 backdrop-blur-md"
>
  {#if submitState === "success"}
    <!-- SUCCESS STATE -->
    <div class="success-view text-center py-20 md:py-32 px-6 md:px-12">
      <div
        class="checkmark-wrap mx-auto mb-10 w-20 h-20 border-2 border-brass rounded-full flex items-center justify-center"
      >
        <svg
          width="32"
          height="32"
      ...
      <h3
        class="font-heading text-silver text-3xl md:text-5xl tracking-cinematic uppercase mb-6"
      >
        MAMY TO.
      </h3>
      <p
        class="font-sans text-steel text-base md:text-lg leading-relaxed max-w-xl mx-auto mb-3"
      >
  ...
  {:else if submitState === "error"}
    ...
    <div class="error-view text-center py-16 md:py-24 px-6 md:px-12">
      <div
        class="mx-auto mb-8 w-16 h-16 border-2 border-brass/60 rounded-full flex items-center justify-center"
      >
        <svg
          width="24"
          height="24"
      ...
      <h3
        class="font-heading text-silver text-2xl md:text-3xl tracking-cinematic uppercase mb-4"
      >
        COŚ POSZŁO NIE TAK.
      </h3>
      <p
        class="font-sans text-steel text-base leading-relaxed max-w-lg mx-auto mb-8"
      >
        Nie udało nam się zapisać Twojego zgłoszenia. Spróbuj ponownie albo
        napisz bezpośrednio na adres mailowy. Twoje dane nie zostały utracone.
      </p>
  ...
  {:else}
    ...
    <header class="px-6 md:px-12 pt-8 md:pt-10 pb-6 border-b border-white/10">
      <div class="flex items-center justify-between mb-3">
        <span
          class="font-mono text-brass text-[11px] md:text-xs tracking-[0.3em] uppercase"
        >
          KROK {String(currentStep).padStart(2, "0")} / 0{totalSteps}
        </span>
        <span
          class="font-mono text-steel text-[11px] md:text-xs tracking-[0.3em] uppercase"
        >
      ...
      <div class="relative h-px bg-white/10 overflow-hidden">
        <div
          ...
          style="width: {(currentStep / totalSteps) *
            100}%; transition: width 0.4s cubic-bezier(0.16, 1, 0.3, 1);"
        ></div>
      </div>
    </header>
    ...
    <div
      bind:this={viewport}
      class="briefflow-viewport grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 p-6 md:p-12 lg:p-16 min-h-128"
    >
      <!-- LEWA kolumna: kontekst i edukacja -->
      <aside class="lg:col-span-5">
        <p
          class="font-mono text-brass text-[11px] tracking-[0.3em] uppercase mb-4"
        >
          {stepHeadings[currentStep].label}
        </p>
        <h3
          class="font-heading text-silver text-2xl md:text-3xl lg:text-4xl tracking-[0.04em] uppercase leading-[1.1] mb-6 max-w-md text-balance"
        >
          {stepHeadings[currentStep].heading}
        </h3>
        <p
          class="font-sans text-steel text-base md:text-lg leading-relaxed max-w-md"
        >
        ...
        <div class="hidden lg:flex items-end h-32 mt-12">
          <div
            class="w-px h-full bg-linear-to-b from-transparent to-brass/40"
          ></div>
        </div>
      </aside>
            ...
            {#each diagnosisOptions as option}
              ...
              >
                <div class="flex items-start gap-5">
                  <span
                    class="font-mono text-brass text-base md:text-lg tracking-wider shrink-0 pt-1"
                  >
                    0{diagnosisOptions.indexOf(option) + 1}
                  </span>
                  <div class="flex-1">
                    <h4
                      class="font-heading text-silver text-base md:text-lg tracking-wider uppercase leading-snug mb-1"
                    >
                      {option.title}
                    </h4>
        ...
        {:else if currentStep === 2}
          ...
          <div class="space-y-6">
            <div>
              <label
                for="industry"
                class="block font-mono text-brass text-[11px] tracking-[0.25em] uppercase mb-2"
                >BRANŻA</label
              >
              <select
                id="industry"
            ...
            <div>
              <p
                class="font-mono text-brass text-[11px] tracking-[0.25em] uppercase mb-3"
              >
                WIELKOŚĆ ZESPOŁU
              </p>
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="flex justify-between mt-2">
                <span
                  id="problem-counter"
                  class="font-mono text-steel/80 text-[11px] tracking-widest"
                >
                  {formData.problem.length} znaków {formData.problem.length < 30
                    ? "(min. 30)"
                    : ""}
                </span>
                <button
            ...
            {#if formData.helperOpen}
              <div class="space-y-3 p-4 border border-brass/20 bg-void/30">
                <p
                  class="font-mono text-brass text-[11px] tracking-widest uppercase mb-2"
                >
                  Pomocnicze pytania:
                </p>
                {#each [{ key: "frequency", label: "Jak często pojawia się ten problem?" }, { key: "people", label: "Ile osób się tym zajmuje?" }, { key: "time", label: "Ile czasu zajmuje za każdym razem?" }, { key: "tools", label: "Jakich narzędzi używacie do tego dziś?" }, { key: "tried", label: "Co już próbowaliście?" }] as q}
                  <div>
                    <label
                      for={`helper-${q.key}`}
                      class="block text-steel text-xs mb-1">{q.label}</label
                    >
                    <input
                      id={`helper-${q.key}`}
                      type="text"
                      bind:value={
                        formData.helperAnswers[
                          q.key as keyof typeof formData.helperAnswers
                        ]
                      }
            ...
            <div class="border border-dashed border-brass/30 p-6 text-center">
              <p
                class="font-mono text-steel/90 text-xs tracking-widest uppercase"
              >
                OPCJONALNIE: PRZECIĄGNIJ TU SCREENSHOTY LUB DOKUMENTY
              </p>
              <p
                class="font-mono text-steel/90 text-[10px] tracking-widest uppercase mt-1"
              >
        ...
        {:else if currentStep === 4}
          ...
          <div class="space-y-8">
            <div>
              <label
                for="hours"
                class="block font-mono text-brass text-[11px] tracking-[0.25em] uppercase mb-3"
              >
              ...
              <div class="flex items-center gap-6">
                ...
                />
                <span
                  class="font-heading text-brass text-2xl md:text-3xl min-w-20 text-right"
                >
            ...
            <div>
              <p
                class="font-mono text-brass text-[11px] tracking-[0.25em] uppercase mb-3"
              >
                CZY PROBLEM ROŚNIE ZE SKALĄ FIRMY?
              </p>
            ...
            {#if annualCost > 0}
              <div
                class="annual-cost-card border border-brass/40 bg-brass/5 p-5 md:p-6 text-center"
              >
                <p
                  class="font-mono text-brass text-[10px] md:text-xs tracking-[0.3em] uppercase mb-2"
                >
                  PRZYBLIŻONY KOSZT ROCZNY
                </p>
                <p
                  class="font-heading text-silver text-3xl md:text-5xl tracking-tight"
                >
                  ~ {formatPLN(annualCost)} zł
                </p>
                <p
                  class="font-mono text-steel text-[10px] tracking-widest uppercase mt-3"
                >
            ...
            <div>
              ...
              <div class="space-y-2">
                {#each triedOptions as t}
                  <label
                    class="flex items-start gap-3 p-3 border bg-void/40 transition-colors duration-200 ease-out hover:border-brass/40 interactive cursor-none"
                  ...
                  >
                    ...
                    />
                    <span class="font-sans text-silver text-sm leading-relaxed"
                      >{t}</span
                    >
                  </label>
                {/each}
            ...
            <div>
              ...
              <div class="space-y-2">
                {#each scopeOptions as s}
                  ...
                  >
                    <div
                      class="font-heading text-silver text-base tracking-wider uppercase mb-1"
                    >
                      {s.label}
                    </div>
          ...
          <div class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                />
                <p
                  class="font-mono text-steel/80 text-[10px] tracking-widest mt-1"
                >
                  JEŚLI WOLISZ ROZMOWĘ ZAMIAST MAILA
                </p>
              </div>
              <div>
                <label
            ...
            <div>
              <label
                for="nip"
                class="block font-mono text-brass text-[11px] tracking-[0.25em] uppercase mb-2"
                >NIP (opcjonalnie, do faktury)</label
              >
              <input
                id="nip"
                type="text"
            ...
            <div class="space-y-3 pt-4 border-t border-white/10">
              <label
                class="flex items-start gap-3 interactive cursor-none"
              >
                <input
                  type="checkbox"
                  bind:checked={formData.agreedPrivacy}
                  class="mt-1 accent-brass"
                />
                <span class="font-sans text-silver/85 text-sm leading-relaxed">
                  Wyrażam zgodę na przetwarzanie danych zgodnie z <button
                    type="button"
                    class="text-brass underline hover:text-brass-light interactive cursor-none"
                    onclick={() =>
                      document.getElementById("open-privacy")?.click()}
              ...
              >
                <input
                  type="checkbox"
                  bind:checked={formData.agreedTerms}
                  class="mt-1 accent-brass"
                />
                <span class="font-sans text-silver/85 text-sm leading-relaxed">
                  Akceptuję <button
                    ...
                    class="text-brass underline hover:text-brass-light interactive cursor-none"
                    onclick={() =>
                      document.getElementById("open-terms")?.click()}
                    >Regulamin</button
                  >
                </span>
              </label>
    ...
    <!-- NAWIGACJA -->
    <footer
      class="px-6 md:px-12 pb-8 md:pb-10 pt-6 border-t border-white/10 flex items-center justify-between gap-4"
    >
      <button
        type="button"
